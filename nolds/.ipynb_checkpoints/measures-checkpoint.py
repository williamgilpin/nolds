...
def is_univariate(arr):
  return len(np.squeeze(arr)) == 1
...
def sampen(data, emb_dim=2, tolerance=None, dist=rowwise_chebyshev,
  ...
  Returns:
    emb_dim (int):
      the embedding dimension (length of vectors to compare) for univariate 
      time series
    float:
...
def corr_dim(data, emb_dim=3, rvals=None, dist=rowwise_euclidean,
  ...
  Explanation of the algorithm:
    ...
    If a one-dimensional (scalar) time series is passed, a delay embedding is
    created. Therefore, before calculating C(r), a delay embedding of the time
  ...
  Kwargs:
    emb_dim (int):
      The embedding dimension for univariate time series.
